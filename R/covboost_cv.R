...
covboost_cv <- function(x, learning_rate=0.5, niter=1000, nfolds=10, cores=1)
{
    ...
    for(k in 1:nfolds)
    {
        ...
        for(i in 2:(niter+1))
        {
            ...
            ## Avoid complaining about symmetry: It is precision!
            cvnll_i_k <- suppressWarnings(try(-sum(.dmvnorm_arma_mc(x[holdout[[k]],], rep(0,p), Bk, logd = TRUE, cores=cores)), silent = T))
